...
{
    Task<List<SegmentModel>> GetAllAsync();
}

public class SegmentService : ISegmentService
{
    private const string CacheKey = "segments";
    private readonly HttpClient _httpClient;
    private readonly ILogger<SegmentService> _logger;
    private readonly IMemoryCache _cache;

    private static readonly Uri Endpoint = new("https://backoffice.brisa-online.com/brisarestws/rest/brisasegments");

    public SegmentService(HttpClient httpClient, ILogger<SegmentService> logger, IMemoryCache cache)
    {
    ...
    {
        if (_cache.TryGetValue(CacheKey, out List<SegmentModel>? cached) && cached.Count > 0)
            return cached;
        ...
        try
        {
            ...
            if (doc.RootElement.TryGetProperty("brisasegment", out var arr) && arr.ValueKind == JsonValueKind.Array)
            {
                foreach (var item in arr.EnumerateArray())
                {
                    try
                    {
                        list.Add(SegmentModel.FromJson(item));
                    }
                    catch (Exception exItem)
                    {
                        _logger.LogWarning(exItem, "Segment parse error - skipping item");
                    }
                }
            }
