...
{
    Task<List<SeasonModel>> GetAllAsync();
}
...
public class SeasonService : ISeasonService
{
    ...
    private static readonly Uri Endpoint = new("https://backoffice.brisa-online.com/brisarestws/rest/brisaseasons");

    public SeasonService(HttpClient httpClient, ILogger<SeasonService> logger, IMemoryCache cache)
    {
    ...
    {
        if (_cache.TryGetValue(CacheKey, out List<SeasonModel>? cached) && cached.Count > 0)
            return cached;
        ...
        try
        {
            ...
            if (doc.RootElement.TryGetProperty("brisaseason", out var arr) && arr.ValueKind == JsonValueKind.Array)
            {
                foreach (var item in arr.EnumerateArray())
                {
                    try
                    {
                        list.Add(SeasonModel.FromJson(item));
                    }
                    catch (Exception exItem)
                    {
                        _logger.LogWarning(exItem, "Season parse error - skipping item");
                    }
                }
            }
